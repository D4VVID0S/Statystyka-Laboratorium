...
wiek <- read.csv2("Temat 2/wiek.csv", header = T)
EX <- mean(wiek$wiek)
###mean(wiekDane$wiek)
varX<-var(wiek$wiek)
...
mean(wiek$wiek) #orednia arytmetyczna
weighted.mean(wiek$wiek) #orednia wa?ona
harmonic.mean(wiek$wiek) #srednia harmoniczna
geometric.mean(wiek$wiek) #orednia geometryczna 
...
cat("Osoby w wieku 31-40 lat stanowi1",z1 , "% pracownik�w firmy.") # message("Osoby w wieku 31-40 lat stanowi1 ", z1 , "% pracownik�w firmy.")
cat("Osoby nieprzekraczaj1ce 40 lat stanowi1", z2 , "% pracownik�w firmy.")


# Obserwacje odstaj1ce?
...
plot(as.numeric(names(dystrybuanta)), dystrybuanta, type="s", main="Dystrybuanta empiryczna - szereg punktowy", xlab="Wydajnooa", ylab="P-stwo")
...
plot(dystrybuantaPrzedzialowa, type="s", main="Dystrybuanta empiryczna - szereg przedzia3owy", xlab="Wydajnooa - przedzia3y", ylab="P-stwo", xaxt="n")
